         ...
         while True:
             ...
             if Pass + Defer + Fail != 120:
                 ...
                 if Pass == 120:
                    print("Progress")
                    Progress_count += 1
                    dictionary_1[UOW_number] = f"Progress - {Pass}, {Defer}, {Fail}"
                    
                 elif Pass == 100:
                    print("Progress (module trailer)")
                    Trailer_count += 1
                    dictionary_1[UOW_number] = f"Trailer - {Pass}, {Defer}, {Fail}"
                    
                 elif 0 <= Pass <=80 and 0<= Fail <= 60:
                    print("Module retriever")
                    Retriever_count += 1
                    dictionary_1[UOW_number] = f"Module retriever - {Pass}, {Defer}, {Fail}"
                    
                 else:
                    print("Exclude")
                    Excluded_count += 1
                    dictionary_1[UOW_number] = f"Exclude - {Pass}, {Defer}, {Fail}"
